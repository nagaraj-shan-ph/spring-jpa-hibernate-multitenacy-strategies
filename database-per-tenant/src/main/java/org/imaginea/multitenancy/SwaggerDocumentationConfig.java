...
public class SwaggerDocumentationConfig {
  ...
  public Docket customImplementation() {
    return new Docket(DocumentationType.SWAGGER_2).select().apis(RequestHandlerSelectors.basePackage("org.imaginea")).build().useDefaultResponseMessages(false)
        .pathProvider(new BasePathAwareRelativePathProvider(swaggerDocBasePath)).globalOperationParameters(Lists.newArrayList(new ParameterBuilder()
            .name(TenantInterceptor.TENANT_HEADER).description("Tenant Id").modelRef(new ModelRef("string")).parameterType("header").required(true).build()));
  }
